...
module.exports = resumable = function(temporaryFolder, outputFolder) {
    var $ = this;
    ...
    var getEncrpytFilename = function(filename, totalSize, chunkNumber) {

        return path.join($.temporaryFolder, './resumable-' + totalSize + '-' + filename + '.' + chunkNumber);
    }
    ...
    $.get = function(req, callback) {
        ...
        if (validateRequest(chunkNumber, chunkSize, totalSize, identifier, filename) == 'valid') {
            var chunkFilename = getEncrpytFilename(filename, totalSize, chunkNumber);
    ...
    $.post = function(req, callback) {
        ...
        if (validation == 'valid') {
            ...
            var encrpytChunkFilename = getEncrpytFilename(filename, totalSize, chunkNumber);

            // Save the chunk (TODO: OVERWRITE)
            encryptor.encryptFile(files[$.fileParameterName].path, encrpytChunkFilename, $.key, $.alg_options, function(err) {
                ...
                var testChunkExists = function() {
                    fs.exists(getEncrpytFilename(filename, totalSize, chunkNumber), function(exists) {
                        if (exists) {
                            currentTestChunk++;
